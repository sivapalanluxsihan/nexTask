...
| :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :---------- | :------ |
| T0.1    | **Monorepo Scaffolding** \- Initialize pnpm workspaces. \- Create root pnpm-workspace.yaml. \- Scaffold server, client, and types directories.                             | Member A    | Pending |
| T0.2    | **Backend & API Config** \- Initialize Express with TypeScript. \- Configure tsoa for automated Swagger generation. \- Link shared TypeScript types to the server package. | Member A    | Pending |
| T0.3    | **Database & Schema** \- Initialize PostgreSQL connection. \- Set up Prisma ORM. \- Define base User and Task models in schema.prisma.                                     | Member A    | Pending |
| T0.4    | **Frontend Foundations** \- Initialize React via Vite with TypeScript. \- Install and configure Tailwind CSS. \- Set up shadcn/ui and the global theme.                    | Member A    | Pending |
| T0.5    | **Containerization** \- Create production and development Dockerfiles. \- Configure docker-compose.yml for multi-service orchestration.                                    | Member A    | Pending |
...
| Task ID | Task Description                                                                                                                                                                                              | Assigned To | Status  |
| :------ | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | :---------- | :------ |
| T1.1    | **Authentication Engine** \- Implement Argon2 password hashing utilities. \- Build JWT generation and verification logic. \- Create Login and Registration endpoints.                                         | Member A    | Pending |
| T1.2    | **Task Services** \- Create Prisma-based CRUD services for tasks. \- Implement controllers for fetching, creating, and updating tasks.                                                                        | Member B    | Pending |
...
| T1.4    | **User Onboarding Flows** \- Build the "First Login" password reset logic. \- Implement the User Profile edit interface. \- Enforce password complexity frontend validation.                                  | Member D    | Pending |
| T1.5    | **Error Infrastructure** \- Create a global Error Middleware for structured responses. \- Standardize Error objects (Code, Message, Description). \- Implement frontend toast notification system for errors. | Member E    | Pending |
...
| T3.4    | **Admin Management** \- Build the searchable User Management dashboard. \- Implement role assignment and user deactivation logic. \- Create an audit log view for administrative changes.                                | Member D    | Pending |
